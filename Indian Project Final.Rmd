...
plot(train_knn) #plots k against the accuracy for various tuning parameters
train_knn$bestTune #shows the best k (100)
...
mean(y_hat_glm == test$Disease)
y_hat_glm <- as.matrix(y_hat_glm)
colnames(y_hat_glm) <- 'GLM'
```

```{r}
#train multinom, adaboostt, svmlinear
set.seed(11)
...
final_ensemble <- cbind(pred_matrix[,2], y_hat_gam, y_hat_rf2, y_hat_glm) #ensemble model was designed with the models that obtained top 3 highest accuracies

one <- rowMeans(final_ensemble == 1)
y_hat_final_ensemble <- ifelse(one > 0.5, 1, 0)
mean(y_hat_final_ensemble == test$Disease)
```
